...
// Function to localize formulas for different sheet locales
function localizeFormula(formula, locale = 'en-US') {
  debugLog('Localizing formula', { original: formula, locale });
  
  if (!formula || typeof formula !== 'string') {
    return formula;
  }
  
  // Detect if localization is needed based on locale
  const needsSemicolon = locale !== 'en-US' && locale !== 'en-CA' && locale !== 'en-AU';
  
  if (needsSemicolon) {
    // Replace commas with semicolons in formula arguments
    // This regex matches commas that are function argument separators, not those within quoted strings
    const localizedFormula = formula.replace(/,(?=(?:[^"]*"[^"]*")*[^"]*$)/g, ';');
    debugLog('Formula localized', { original: formula, localized: localizedFormula });
    return localizedFormula;
  }
  
  debugLog('No localization needed for locale', { locale });
  return formula;
}
...
function createWeeklySummaryEU(spreadsheet, rawData) {
  ...
  try {
    ...
    // Year subheaders (row 10) - Create conditional formulas that work with EU locale (semicolon notation)
    const previousYearFormula = localizeFormula(`=IF($B$4="Yes","",${previousYear})`, 'nl-NL');
    const yoyFormula = localizeFormula(`=IF($B$4="Yes","","YoY")`, 'nl-NL');
    const wowFormula = localizeFormula(`=IF($B$5="Yes","","WoW")`, 'nl-NL');
    ...
    if (formulaData.length > 0) {
      ...
      // Apply localization to all formulas before writing (EU locale)
      const localizedFormulaData = formulaData.map(row => 
        row.map(cell => typeof cell === 'string' && cell.startsWith('=') ? localizeFormula(cell, 'nl-NL') : cell)
      );
      
      const summaryRange = summarySheet.getRange(11, 1, formulaData.length, columnCount);
      summaryRange.setValues(localizedFormulaData);
      
      debugLog('Formula data written successfully to EU sheet with localization');
    }
