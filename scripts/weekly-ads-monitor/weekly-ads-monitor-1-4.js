...
function main() {
  try {
    Logger.log('🚀 STARTING WEEKLY ADS MONITOR v1.4');
    ...
    Logger.log('🎉 SUCCESS! Weekly Ads Monitor v1.4 completed');
...
function createWeeklySummary(spreadsheet, rawData) {
  try {
    ...
    summarySheet.getRange('A1').setValue('Weekly Ads Monitor v1.4');
    ...
    const mainHeaders = ['Week Number', 'Week Start', 'Clicks', '', '', 'Average CPC', '', '', 'Cost', '', '', 'Conversions', '', '', 'Conversion Value', '', '', 'ROAS', '', ''];
    summarySheet.getRange('A6:T6').setValues([mainHeaders]);
    
    // Year subheaders (row 7) - including Index YoY columns
    const yearHeaders = ['', '', currentYear, previousYear, 'Index YoY', currentYear, previousYear, 'Index YoY', currentYear, previousYear, 'Index YoY', currentYear, previousYear, 'Index YoY', currentYear, previousYear, 'Index YoY', currentYear, previousYear, 'Index YoY'];
    const yearHeaderRange = summarySheet.getRange('A7:T7');
    ...
    summarySheet.getRange('R7:S7').setNumberFormat('0'); // ROAS years
    
    // Merge cells for main headers
    try {
      summarySheet.getRange('A6:A7').merge(); // Week Number
      summarySheet.getRange('B6:B7').merge(); // Week Start
      ...
      summarySheet.getRange('R6:T6').merge(); // ROAS
    ...
    for (let i = 0; i < maxWeeks; i++) {
      ...
      const roasIndexFormula = `=IF(S${rowNum}>0,(R${rowNum}/S${rowNum})*100,0)`;
      ...
      const formulaRow = [
        ...
        roasCurrentFormula,                 // R: ROAS Current Year (2025)
        roasPreviousFormula,                // S: ROAS Previous Year (2024)
        roasIndexFormula                    // T: ROAS Index YoY
      ];
    ...
    if (formulaData.length > 0) {
      const summaryRange = summarySheet.getRange(8, 1, formulaData.length, 20);
      summaryRange.setValues(formulaData);
    }
    
    // Format main header (row 6 only)
    const mainHeaderRange = summarySheet.getRange(6, 1, 1, 20);
    ...
    const yearSubHeaderRange = summarySheet.getRange(7, 1, 1, 20);
    ...
    if (formulaData.length > 0) {
      ...
      for (let i = 0; i < dataRowCount; i++) {
        ...
        if (i % 2 === 1) { // Every other row (odd rows)
          const rowRange = summarySheet.getRange(rowNum, 1, 1, 20);
      ...
      // ROAS group border (R6:T through last data row)
      const roasGroupRange = summarySheet.getRange(6, 18, totalRows, 3); // Columns R-T
      ...
      const weekNumberRange = summarySheet.getRange(8, 1, dataRowCount, 1);
      ...
      const weekStartRange = summarySheet.getRange(8, 2, dataRowCount, 1);
      ...
      // ROAS (Columns R & S) - Number format with 2 decimals, center aligned
      const roasRange = summarySheet.getRange(8, 18, dataRowCount, 2);
      roasRange.setNumberFormat('#,##0.00');
      roasRange.setHorizontalAlignment('center');
      
      // ROAS Index (Column T) - Percentage format, center aligned
      const roasIndexRange = summarySheet.getRange(8, 20, dataRowCount, 1);
      ...
      const indexColumns = [5, 8, 11, 14, 17, 20]; // E, H, K, N, Q, T
    ...
    // Set equal widths for ROAS columns (R, S, T)
    summarySheet.setColumnWidth(18, 80);  // ROAS 2025
    summarySheet.setColumnWidth(19, 80);  // ROAS 2024
    summarySheet.setColumnWidth(20, 80);  // ROAS Index YoY
